...
{
	Vec2i screenRes = getWindowResolution();
	Vec2i workingRes = ScaleSystem::getInstance()->getWorkingResolution();
...
void GraphicsManager::update()
{
	...
	if(activeScene)
	{
		...
		if(camera)
		{
			auto component = camera->getComponent<CameraComponent::GUID>();
			if (component)
			{
...
{
	return mViewportResolution.x;
}

int32 GraphicsManager::getViewportHeight() const
{
	return mViewportResolution.y;
}
...
const Vec2i& GraphicsManager::getWindowResolution() const
{
	return mScreenResolution;
}

const Vec2i& GraphicsManager::getViewportResolution() const
{
	return mViewportResolution;
}

const Vec2i& GraphicsManager::getScreenResolution() const
{
...
{
	mScreenResolution.x = width;
	mScreenResolution.y = height;
	calculateViewPort();
}
